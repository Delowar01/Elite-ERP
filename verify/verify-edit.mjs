...
const email=`ed_${uniq()}@test.dev`;
await p.goto(`${BASE}/register`,{waitUntil:"networkidle"});
await p.fill("#orgName",`ED Org ${uniq()}`);await p.fill("#name","ED");await p.fill("#email",email);await p.fill("#password",`Zx9$mQ${uniq()}vK!ray`);
...
await p.goto(`${BASE}/sales/quotations/${id}/edit`,{waitUntil:"networkidle"});await p.waitForTimeout(500);
