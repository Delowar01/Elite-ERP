...
function DialogOverlay({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      className={cn(
        "fixed inset-0 z-50 bg-brand-navy-deep/40 backdrop-blur-[2px] data-[state=open]:animate-fade-up",
        className,
      )}
      {...props}
    />
  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        className={cn(
          "fixed left-1/2 top-1/2 z-50 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-line bg-surface-glass p-6 shadow-glass backdrop-blur-xl outline-none",
          className,
        )}
        {...props}
      >
