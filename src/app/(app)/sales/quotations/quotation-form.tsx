...
export function QuotationForm({
  ...
  function submit() {
    startTransition(async () => {
      const result = await createQuotationAction({ title, customerId, issueDate, validUntil, discount, notes, items });
  ...
  return (
    <div className="flex flex-col gap-5 max-w-5xl">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h3 className="text-[19px] font-bold">{t(locale, "Create Quotation")}</h3>
          <div className="text-[12px] text-ink-muted">{t(locale, "Create and send professional quotations to your clients.")}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[1.7fr_1fr] gap-5 items-start">
        <div className="flex flex-col gap-3.5">
          <div className="grid grid-cols-2 gap-3.5">
            ...
            <DocFieldBox label={t(locale, "Quotation Date")} required mono={false}>
              <input
                type="date"
                value={issueDate}
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <DocFieldBox label={t(locale, "Valid Till Date")} required gear mono={false}>
              <input
                type="date"
                value={validUntil}
          ...
          <div>
            <label className="block text-[11.5px] font-semibold text-ink-muted mb-1.5">{t(locale, "Quotation Title")}</label>
            <input
              value={title}
        ...
        <div className="rounded-2xl border border-line bg-linear-to-br from-surface-raised to-surface shadow-elevated flex flex-col items-center justify-center gap-1.5 py-6 text-center">
          {org.logoUrl ? (
            ...
            <div className="size-11 rounded-xl bg-brand-navy flex items-center justify-center text-white font-display font-bold">
              {org.name.slice(0, 1)}
            </div>
          )}
