...
export function ProformaForm({
  ...
  const dirtyForm = useDirtyForm({ title, customerId, issueDate, discount, notes, terms, items, attachments, bankAccountIds, currency, sealOverride, signatureOverride });
  ...
  function submit(andSend: boolean) {
    ...
    start(async () => {
      ...
      dirtyForm.markClean();
      const payload = { title, customerId, issueDate, discount, notes, terms, items, attachments, bankAccountIds, currency, sealUrl: sealOverride, signatureUrl: signatureOverride };
